	...
	int num_dai)
{
	...
	if (dai_drv && num_dai > 0)
		for (i = 0; i < num_dai; i++) {
			...
			snprintf(buf, 50, "%s-%x-%x", dai_drv[i].name, i2cbus_num,
				addr);
			dai_drv[i].name = tfa98xx_devm_kstrdup(dev, (char*)buf);

			pr_info("tfa98xx_append_i2c_address:buf=%s i2cbus=%x", (char*)buf, i2cbus);
			memset(buf, 0x00, sizeof(buf));
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	pr_info("tfa98xx codec registered\n");
...
static int __init tfa98xx_i2c_init(void)
{
	...
	pr_info("---0--TFA98XX driver version %s\n", TFA98XX_VERSION);
	...
	pr_info("---1--TFA98XX driver version %s\n", TFA98XX_VERSION);
	ret = i2c_add_driver(&tfa98xx_i2c_driver);
	pr_info("---2--TFA98XX driver version %s\n", TFA98XX_VERSION);
