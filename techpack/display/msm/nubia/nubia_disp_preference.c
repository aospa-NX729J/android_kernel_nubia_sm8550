        ...
        struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	//nubia_enable_dimming_feature(nubia_display->panel, false);
        ...
        struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &val);
	if (nubia_display->panel->ddic_vendor == RAYDIUM){
		if(val < 336 || val > 4095) {
...
static ssize_t checksum_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	uint32_t val = 0;

	if(!nubia_display->panel->panel_initialized) {
		NUBIA_DISP_ERROR("panel not ready\n");
		return 0;
	}
	...
	NUBIA_DISP_INFO("checksum_store val = %d\n",val);

	if(val == 11 || val == 22) {
		sde_connector_schedule_status_work(nubia_display->drm_conn,false);
		if(val == 11) {
			checksum_temp = nubia_dsi_panel_checksum(nubia_display->panel, 1);
		}else if(val == 22) {
			checksum_temp = nubia_dsi_panel_checksum(nubia_display->panel, 2);
		}else{
			NUBIA_DEBUG ("invalid checksum mode val\n");
		}
		sde_connector_schedule_status_work(nubia_display->drm_conn,true);
		/*usleep_range((50 * 1000), (50 * 1000) + 10);//delay 50ms
		mipi_dsi_dcs_write(dsi, 0xFE, cmd, sizeof(cmd));
		rc = dsi_panel_read_data(dsi, 0x91, data, 1);
		NUBIA_DISP_INFO ("data[0] = 0x%x  ,rc = %d\n", data[0],rc);
		if(rc < 0){
				pr_err("%s: read checksum id error \n", __func__);
		}
		firmware_id = data[0];
		checksum_temp = firmware_id;
		NUBIA_DISP_INFO ("firmware_id = 0x%x\n", firmware_id);
		nubia_dsi_panel_checksum(nubia_display->panel, 0);
		*/
	}
	NUBIA_DISP_INFO(" checksum_temp = %d\n",checksum_temp);
	return size;
}
...
static struct kobj_attribute lcd_disp_attrs[] = {
	...
	__ATTR(checksum, 0664, checksum_show, checksum_store),
